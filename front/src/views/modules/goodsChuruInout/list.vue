<template>
    <div class="main-content">
        <el-dialog :title="goodsChuruInoutTitle" :visible.sync="goodsChuruInoutDialog">
            采购入库订单名:<el-input prefix-icon="el-icon-search" v-model="goodsChuruInoutName" placeholder="采购入库订单名" style="width: 200px" :readonly="isinfo?'readonly':false"></el-input>
            <span v-if="!isinfo" >
                商品信息:
            ...
            <el-table :data="goodsChuruInoutData">
                ...
                <el-table-column property="goodsChuruInoutListNumber" :label="goodsChuruInoutTitle == '入库'?'入库数量':'出库数量'" width="200">
                    <template slot-scope="scope">
                        <el-input-number v-model="scope.row.goodsChuruInoutListNumber" :min="1" :max="goodsChuruInoutTitle == '入库'?1000:scope.row.goodsKucunNumber" size="mini" :disabled="isinfo"></el-input-number>
                    </template>
                </el-table-column>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/shipinkucunguanli/upload/goodsChuruInoutMuBan.xls"
                        >批量导入采购入库数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('goodsChuruInout','导入导出')"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('goodsChuruInout','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('goodsChuruInout','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('goodsChuruInout','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            goodsOptions: [],
            goodsChuruInoutDialog :false,//采购入库订单模态框
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'goods_churu_inout',//当前表表名,
                        sumColum : 'goods_churu_inout_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'goods_churu_inout_name',//分组字符串字段
                        // types : 'goods_churu_inout_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "goods_churu_inout_types",
                //     }
                //     this.$http({
            ...
            getId(id){
                ...
                _this.$http({
                    ...
                    if (data && data.code == 0) {
                        _this.goodsChuruInoutTitle = (data.data.goodsChuruInoutTypes == 1?"出库":"入库");
            ...
            submitGoodsChuruInoutData(){
                ...
                params["map"] =map;
                _this.$http({
                    url: `goodsChuruInout/${_this.goodsChuruInoutTitle == "入库" ? "inGoodsChuruInoutList" : "outGoodsChuruInoutList"}`,
                    method: "post",
                    data: params
                }).then(({ data }) => {
                    if (data && data.code === 0) {
